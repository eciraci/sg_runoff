...
-> TEST: evaluate the usability of this implementation to compute sub-glacial
         runoff over the entire ice sheet in a single step.
...
def main() -> None:
    ...
    project_dir = os.path.join('/', 'Volumes', 'Extreme Pro', 'BedMachine',
                               'output.dir')
    output_dir = make_dir(project_dir, 'subglacial_flow_rich_test')
    output_dir = make_dir(output_dir, 'BedMachineGreenland')
    ...
    bed_path \
        = os.path.join('/', 'Volumes', 'Extreme Pro', 'BedMachine',
                       ...
                       'BedMachineGreenland_bed_EPSG-3413_res150.tiff')
    # - Ice Elevation
    elev_path \
        = os.path.join('/', 'Volumes', 'Extreme Pro', 'BedMachine',
                       'output.dir',
                       'BedMachineGreenland-version_05_surface',
                       'BedMachineGreenland_surface_EPSG-3413_res150.tiff')

    # - Ice Thickness
    thick_path \
        = os.path.join('/', 'Volumes', 'Extreme Pro', 'BedMachine',
                       'output.dir',
                       'BedMachineGreenland-version_05_thickness',
                       'BedMachineGreenland_thickness_EPSG-3413_res150.tiff')
    ...
    thick_input['data'][ice_mask != 1] = np.nan
    bed_input['data'][ice_mask != 1] = np.nan
    elev_input['data'][ice_mask != 1] = np.nan
    ...
    x_vect = bed_input['x_coords']
    y_vect = bed_input['y_coords']
    crs = bed_input['crs']
    res = bed_input['res']
    extent = bed_input['extent']
    ...
    # -
    out_path = os.path.join(output_dir, 'BedMachineGreenland'
                                        '_Stereo_hydro_pot_EPSG-3413'
                                        '_res150.tiff')
    save_raster(hydro_pot, res, x_vect, y_vect, out_path, crs)

    # - Load the computed Hydraulic Potential Mask
    hydro_pot_in = load_raster(out_path)
    ...
    dem = rd.LoadGDAL(bed_path, no_data=-9999)
    ...
    # - Get flow accumulation with no explicit weighting.
    ...
    plt.ylabel('Northing')
    plt.savefig(
        os.path.join(output_dir, f'flow_accumulation_{routing}'
                                 f'_log.{fig_format}'),
        dpi=dpi, format=fig_format)
    plt.close()
