...
def main() -> None:
    ...
    grid = Grid.from_raster(out_path, data_name='hydro_pot')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir,
                             f'flow_direction_grid_D8.{fig_format}'),
                dpi=dpi, format=fig_format)
    plt.close()

    # - Calculate flow accumulation
    acc = grid.accumulation(fdir, dirmap=dirmap, routing='d8', nodata_in=np.nan)
    ...
    branches = grid.extract_river_network(fdir, acc > rn_thresh, dirmap=dirmap)
    ...
    out_f_name = os.path.join(output_dir,
                              'flow_network_D8.shp')
